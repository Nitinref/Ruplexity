...
export async function processQuery(query: string, conversationId: string = 'default') {
  try {
...
function extractSources(result: any): any[] {
  const sources: any[] = [];
      ...
      if (step.toolCalls) {
        step.toolCalls.forEach((call: any) => {
          if (call.result?.results) {
            call.result.results.forEach((item: any) => {
              sources.push({
                id: sources.length + 1,
...
function generateFollowUpQuestions(originalQuery: string, response: string): string[] {
